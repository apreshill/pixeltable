...
A schema file defines one or more models on a `pxt.model_base()`. Each model becomes one table, named by `name=`. The same file can declare `FastAPIRouter` services: apply tables with `pxt schema update` before `pxt service update`. `pxt service example` writes that combined file. `pxt schema example` writes a file covering every construct the schema DSL supports:
...
pxt schema update app.py my_app -n                       # the plan, applying nothing
...
Cloud operations that affect a database's runtime image read configuration from `pixeltable.toml` in the current directory.

#### `[pixeltable.database]` — runtime image config

Read by `pxt db update-runtime`, from `pixeltable.toml` in the current directory if that file declares the section, and from the Pixeltable config file otherwise. Defines which local files to package into the runtime image and, optionally, a custom Pixeltable source branch.
...
- [HTTP Serving Guide](/howto/deployment/serving): application file, `pxt service`, Python `FastAPIRouter` API
