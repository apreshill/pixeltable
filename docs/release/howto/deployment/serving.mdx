...
routes over them, and `pxt service` runs it. Write a file to start from with `pxt service example --out app.py`:
...
class Docs(TableModel, name='docs'):
    doc_id: pxt.Int
    prompt: pxt.String
    result = pxtf.string.upper(prompt)  # a computed column
...
curl -X POST http://127.0.0.1:<port>/insert \
...
<Warning>
`@pxt.query` eagerly evaluates the function body at decoration time. Apply the
application file with `pxt schema update` **before** defining queries that
reference those tables. Keep `@router.post()` for multi-table operations,
conditional logic, or a custom response shape. `FastAPIRouter` extends FastAPI's
`APIRouter`, so hand-written and declarative routes can share the same router.
</Warning>
...
```json
{"id": "abc123", "job_url": "http://127.0.0.1:<port>/_pxt/jobs/abc123"}
```

Poll the `job_url` from the response (`pxt service list` prints the base URL):
